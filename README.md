...
Stage B implemented the trusted `nervos.chat@1` Agent Instance and Run surface plus Anthropic/OpenAI portability. Stage C1 added the durable Job/Attempt/RunEvent schema, C2 activated asynchronous submission plus a separate Worker process, C3 added a durable Worker registry and expired-lease reconciliation: work lost before the execution-start boundary is recovered and re-queued, and work lost after it is closed as ambiguous rather than replayed. One submission is one independent Run: the API returns `202 Accepted` after durable queueing, and a capable Worker executes the immutable provider/model snapshot later. C4 added the automatic execution retry engine: a failure the provider positively declined (a normalized rate limit) becomes a durable `retry_wait` obligation with a stored due instant, and a later compatible Worker executes it with a fresh Attempt — surviving restarts with no scheduler — while ambiguous outcomes such as timeouts stay terminal and are never replayed. C5 added owner cancellation: an owner can cancel a queued, retrying, or running Run, the cancellation is durable before the request returns and does not depend on a live Worker, and a cancelled Run is a distinct terminal lifecycle rather than a failure. C5 also added the Attempt execution-timeout watchdog, which keeps a timed-out Attempt `AMBIGUOUS` and never replays it. Cancellation revokes NervOS authority only — it makes no claim that the remote provider stopped or that billing stopped. C6 made execution concurrency authoritative: global, per-Agent-Instance, and per-provider limits are enforced together at claim time from Jobs that hold a live lease, so no Worker's local configuration can raise the system ceiling, and selection is durable least-recently-served per Agent Instance rather than global FIFO, so a continuously eligible Agent is not bypassed by a backlog that merely happens to be older. A partition that is saturated, or whose provider a Worker cannot run, is skipped instead of blocking the queue head, and admission is bounded by global, per-Agent, and per-provider pending limits. C7 made the kernel observable to its owner without giving observability any authority: `GET /api/v1/runs/{run_id}/events` returns a Run's durable execution timeline, owner-scoped by the same rule as the Run itself, ordered by the Run-local sequence the writers allocated and paginated by a keyset cursor rather than an offset. Run responses also carry a derived read-only execution phase — the Job's own durable status, verbatim — which finally distinguishes a Run that is executing now from one waiting out a retry, and the dashboard shows the timeline incrementally with a polling lifetime that no longer abandons a live Run. C7 added no migration: the existing `UNIQUE(run_id, sequence)` index already serves the pagination query. C8 proved the finished engine composes — integrated acceptance across recovery, retry, cancellation, timeout, all three concurrency dimensions, fairness and backpressure; restart and re-composition; multi-Worker composition; a bounded deterministic 120-Job stress soak; SQLite integrity; and security acceptance. NervOS does **not** guarantee exactly-once remote provider execution: it guarantees fenced durable authority, no blind replay of ambiguous execution, safe retry only for a positively safe outcome, and protection against stale late writes — while remote provider processing, billing, and side effects may still occur after an ambiguous crash, a cancellation, or a timeout.

Stage D — the tool and MCP layer — is COMPLETE. D1 added durable tool, capability, and audit schema; D2 added capability grants and the call-time permission decision; D3 added the tool registry, canonical schema, and the first built-in tools; D4 added provider-neutral tool calling and the Think → Act → Observe loop; D5 added the MCP client/gateway and connection lifecycle; D6 added tool audit, failure semantics, and C3–C6 integration; D7 added integrated acceptance and Stage-D closeout.

Stage E — scheduling, events, and triggers — is COMPLETE. E0 froze architecture, protocol, and safety; E1 added the durable trigger/occurrence domain and migration `0008`; E2 added the scheduler engine (one-time/interval/cron, timezone, misfire, multi-instance, restart); E3 added secure webhook ingress, secret authentication, rotation, and idempotency; E4 added internal events, trigger management, occurrence history, and minimal Automations UI; E5 added integrated acceptance and Stage-E closeout.

Stage F — Conversations, context, and memory — is COMPLETE. F0 froze architecture, protocol, safety, and lifecycle; F1 added durable Conversations, Turns, Messages, and Run linkage; F2 added ContextBuilder, mandatory RunContextSnapshot, deterministic compaction, and context bounds; F3 added scoped AGENT/USER Memory, provenance, and deterministic retrieval; F4 added Conversation and memory lifecycle, versioned editing, deletion, active-data invalidation, retention semantics, and minimal UI; F5 added integrated acceptance and Stage-F closeout.
...
Since Stage C the same single journey also drives the durable execution engine end to end: it observes an accepted Run queued before any Worker may claim it, a pre-start Worker loss recovered exactly once, a scripted rate limit retried after its due instant, an owner cancelling genuinely in-flight work, and the resulting execution timeline rendered in the browser row by row and rebuilt identically after a reload. It never contacts a real provider.
